...
\begin{theorem}[Zhang's metric]
There is a unique symmetric integrable metric $\norm{\cdot}_{\Delta,a}$ over $\mathcal{O}_{C^2}(\Delta)$ such that for any finite field extension $K'/K$ and $x,y \in C(K')$, we have
\begin{enumerate}[(1)]
...
$$\int_{(C_{K'})^{an}} g_x(-) c_1(\mathcal{O}(x),\norm{\cdot}_x) =0.$$
\end{enumerate}
\end{theorem}
\section{uniqueness}
